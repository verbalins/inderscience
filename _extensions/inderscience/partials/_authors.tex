...
\authorA{\sf{$it.name.literal$$if(it.attributes.corresponding)$*$endif$}}
\affA{$for(it.affiliations)$$if(it.department)$$it.department$,\\$endif$$it.name$,\\$it.address$,~$it.city$$if(it.country)$,~$it.country$$endif$$sep$\\and\\$endfor$
$if(it.attributes.corresponding)$\\Email:~\href{mailto:$it.email$}{$it.email$}\\
{\sf{*}}Corresponding author$endif$}